using DBFirstTeach.Models;
using Microsoft.AspNetCore.Mvc;
using System.Diagnostics;

namespace DBFirstTeach.Controllers
{
...
一. 使用DB First建立Model
    1. 把dbStudents.sql程式碼用記事本打開，複製裡面內容後到SSMS中新增查詢並貼上執行
        ，即可建立資料庫dbStudents，資料庫dbStudents中有一個tStudent資料表
    2. 建立專案與資料庫的連線
        a. 使用NuGet 
            => 在專案名稱上按右鍵 > 管理NuGet套件 > 安裝以下套件
            (1) Microsoft.EntityFrameworkCore.SqlServer
            (2) Microsoft.EntityFrameworkCore.Tools
        b. 到SSMS設定登入SQL Server的使用者( 必須測試連線成功 )
        c. 到套件管理器主控台
            => 點擊檢視 > 其他視窗 > 套件管理器主控台
            ，並下指令( 上面的專案跟套件管理器中預設專案要記得調整 )
            指令內容 : Scaffold-DbContext "Data Source=伺服器位址;Database=資料庫名稱;TrustServerCertificate=True;User ID=帳號;Password=密碼" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Models -NoOnConfiguring -UseDatabaseNames -NoPluralize -Force
            指令內容說明 : 
            (1) 伺服器位址是SQL Server的IP或主機名稱或當時我建立使用者時自創的名稱( 這裡是Soma )
            (2) 資料庫名稱是dbStudents
            (3) TrustServerCertificate=True是為了避免SSL憑證驗證問題
            (4) User ID和Password是登入SQL Server的使用者帳號和密碼( 這裡是帳號Soma0617和密碼soma830617 )
            (5) -OutputDir Models是指定產生的模型類別檔案存放在Models資料夾中
            (6) -NoOnConfiguring表示不自動產生OnConfiguring方法
            (7) -UseDatabaseNames表示使用資料庫中的原始名稱
            (8) -NoPluralize表示不將類別名稱轉為複數形式
            (9) -Force表示強制覆蓋已存在的檔案
        d. 若連線成功的話，會看到Build succeeded.字眼
            ，並在Models資料夾裡看到dbStudentsContext.cs( 描述資料庫 )及tStudent.cs( 描述資料表 )
        e. 在dbStudentsContext.cs裡寫連線到資料庫的底下兩段程式碼 : 
        ...
        f. 在dbStudentsContext.cs裡撰寫一個空的建構子
                public dbStudentsContext()
                {
                }

二.  製作自動生成的tStudent資料表的CRUD功能
    1. 使用Scaffold方法讓Visual Studio自動建立出tStudent資料表的CRUD功能
        ，包含Controllers中的tStudentController以及Views中的Index.cshtml、Create.cshtml、Edit.cshtml
        、Delete.cshtml、Details.cshtml等view的程式碼
        a. 在Controllers資料夾上按右鍵 > 加入 > 控制器
        b. 選擇使用EntityFramework執行檢視的MVC控制器，按下加入按鈕
        c. 在對話方塊中設定如下 : 
            (1) 模型類別 : tStudent(MyModel_DBFirst.Models)
            (2) 資料內容類別 : dbStudentsContext(MyModel_DBFirst.Models)
            (3) 勾選產生檢視
            (4) 勾選參考指令碼程式庫
            (5) 勾選使用版面配置頁
            (6) 控制器名稱使用預設即可( tStudentsController )
            (7) 按下新增按鈕
        d. 此時Visual Studio會進行Scaffolding動作，將產出一個tStudentsController
            ，包含Controllers中的tStudentController以及Views中的Index.cshtml、Create.cshtml、Edit.cshtml
            、Delete.cshtml、Details.cshtml等view的程式碼
        備註 : 
            Index.cshtml( 清單範本 )
            ...
            Edit.cshtml( 修改範本 )
            Delete.cshtml( 刪除範本 )
            Details.cshtml( 詳細資訊範本 )
    2. 修改tStudentsController的內容
        a. 建立DbContext物件的程式碼
    ...
    補充 : 因為自動生成的Controller寫法是依賴注入( Dependency Injection, DI )
            ，目前我們尚未學到，因此先用一般new物件的寫法
    3. 執行Index View進行CRUD功能測試
    補充 : Visual Studio自動建立出的tStudentController，預設會使用依賴注入( Dependency Injection )的寫法
            不過一開始我們先不使用依賴注入的寫法，因此我們需修改一些步驟
            ，例如( 一.2.e. )以及( 二.2. )等步驟的程式碼後才能正常執行

三. 撰寫模型內容
    1. 打開Model中tStudent.cs檔案
    2. 撰寫在View上顯示的欄位內容( 需先寫上using System.ComponentModel.DataAnnotations )
    3. 撰寫在表單上的欄位驗證規則
        a. 常用的驗證器 Required、StringLength、RegularExpression、Compare、EmailAddress、Range、DataType
            (1) Required : 必填欄位
            (2) StringLength : 資料字數
            (3) RegularExpression : 資料格式( 正規表達式 )
            (4) Compare : 與其它欄位比較是否相等
            (5) EmailAddress : 是否是E-mail格式
            (6) Range : 限制所填的範圍

4. 在tStudentsController檔案的Post Create Action中撰寫檢查學號是否重複的程式碼
5. 在Create View檔案中補上呈現學號重複的錯誤訊息
